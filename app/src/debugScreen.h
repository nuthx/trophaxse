...
#define CLEARSCRN(H, toH, W, toW)     \
    for (int h = H; h < toH; h++)     \
        for (int w = W; w < toW; w++) \
            ((uint32_t *)base)[h * SCREEN_FB_WIDTH + w] = colorBg;
...
static size_t psvDebugScreenEscape(const unsigned char *str)
{
    for (unsigned i = 0, argc = 0, arg[32] = {0}; argc < (sizeof(arg) / sizeof(*arg)) && str[i] != '\0'; i++)
        ...
        case 'K':
            if (arg[0] == 0)
                CLEARSCRN(coordY, coordY + F.size_h, coordX, SCREEN_WIDTH); //from curosr to end
            if (arg[0] == 1)
                CLEARSCRN(coordY, coordY + F.size_h, 0, coordX); //from begining to cursor
            if (arg[0] == 2)
                CLEARSCRN(coordY, coordY + F.size_h, 0, SCREEN_WIDTH); //whole line
            if (str[i] == 'K')
                return i;
            if (arg[0] == 0)
                CLEARSCRN(coordY, SCREEN_HEIGHT, 0, SCREEN_WIDTH);
            if (arg[0] == 1)
                CLEARSCRN(0, coordY, 0, SCREEN_WIDTH);
            if (arg[0] == 2)
                CLEARSCRN(0, SCREEN_HEIGHT, 0, SCREEN_WIDTH);
            return i;
        case 'm': // Color
            if (!arg[0])
            {
            ...
            for (unsigned c = 0; c <= argc; c++)
            {
                ...
                if (mode != 3 && mode != 4 && mode != 9 && mode != 10)
                    continue; //skip unsported modes
                if (unit == 9)
                { // reset FG or BG
                    *color = mode & 1 ? defaultFg : defaultBg;
                }
                else if ((unit == 8) && (arg[c + 1] == 5))
                { // 8bit : [0-15][16-231][232-256] color map
                    c += 2;
                    *color = arg[c] <= 15 ? FROM_3BIT(arg[c], mode < 9) : arg[c] >= 232 ? FROM_GREY(arg[c] - 232) : FROM_6BIT(arg[c] - 16);
                }
                ...
                { // 24b color space
                    ...
                    *color = FROM_3BIT(unit, mode < 9); // standard 8+8 colors
            }
            return i;
        }
    return 0;
...
int psvDebugScreenPuts(const char *_text)
{
    ...
    for (c = 0; text[c]; c++)
    {
        ...
        else if ((t == '\e') && (text[c + 1] == '['))
        {
            ...
            if (coordY < 0)
                coordY = 0; // prevent 0-based coordiate from producing a negativ X/Y
            continue;
        }
        ...
        for (int row = 0, mask = 1 << 7; row < F.height; row++, vram += SCREEN_FB_WIDTH)
        {
            for (uint32_t *pixel = vram, col = 0; col < F.width; col++, mask >>= 1)
            {
                if (!mask)
                {
                    font++;
                    mask = 1 << 7;
                } // no more mask : we exausted this byte
